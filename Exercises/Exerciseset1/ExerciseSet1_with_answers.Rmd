...
1. The code is done in dataSet1.R file - the relevant part of the code is:
...
round(100*sum(StockReturns[,2]),1)   for cumulative return = 577.2

round(100*mean(StockReturns[,2]),3)  for avg daily return = 0.149

round(100*sd(StockReturns[,2]),3)    for standard deviation = 2.426
...
change start date in dataSet1.r
...
**Your Answers here:**
1.
Functions check how many of of first 20 rows of stockreturns are different than first 20 rows in CSV - the result is 20 (all) there might be some rounding issue here, because normally there should be all equal hence the result should be 0
