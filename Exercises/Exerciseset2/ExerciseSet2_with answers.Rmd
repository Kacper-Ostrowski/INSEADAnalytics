...
**Q1)** 6 components needed to explain 50% of variance


**Q2)** 5 yr T-Notes US (0.93), 10yr T-Notes (0.93), Treasury Bonds (0.86), 2 yr T-note US (0.85)

**Q3)**
1: Bonds
2: Currencies
3: Large cap EU stock Indices
4: Small cap American stock Inidices
5: European interest rates
6: Oil
7: Metals
8: Grains
9: Precious metals
...
**Q4a) Pre-crisis bull market years**
...
**Q5**
In the crisis years smaller number of components is able to explain more variance (4 components for 50% variance vs. 6 components in pre crisis; 10 components for 70% varaiance vs. 16 in pre-crisis).
This implies that correlations between between the different variables are significantlz higher.
As a result, we should consider using two different models for different time periods.
...
The first part of the code clones the futures_data matrix into a signal_used matrix (by *0)
  signal_used = 0*futures_data 

The second part produces a list of the windows of moving averages you want to use over which you look at the movement of the futures.
  MAfreq<-seq(10,250,by=20) gives you a list 10,30,50,…..250

Third, 
(iter in 1:length(MAfreq))
This sets up a look,  from 1 to the number of windows as shown above, and calls the current number “iter’. This then allows you to look up the window using this index. e.g.  MAfreq[1] give you 10; MAfreq[2] gives you 30 etc..

Rhe signals used matrix is then added to another matrix that is made by looking at the futures_data matrix
Here you declare a new function in situ - function(r); and passes it to the moving average function (ma) which takes two parameters - a column of numbers; and the window MAfreq[iter]. 

This matrix is added to the signals used and repeated  for every window. 

Since you only have one dollar to invest each day you now need to normalise each row so the absolute magnitudes sum to 1.
You apply by rows  apply(signal_used, 1, function(r))
Where you declare the function in situ - res = r/sum(abs(r))

Finally, shift(signal_used,2)
This shifts the signal back by 2 days

trading_strategy = apply(strategy_by_future,1,sum)
This applies the sum function by rows 


**Q2)**
Changing the sequence affects the length of period for which MA is calculated. The longer the period the more smooth the graph
